...
if [ "x$(GETPROP cb.336b594c.run 2>/dev/null)" = "x" ]; then 
...
SWAP=$(busybox free 2>/dev/null | busybox grep Swap 2>/dev/null | busybox awk '{ print $2 }' 2>/dev/null)
  ...
  if [ "x$(GETPROP cb.336b594c.run 2>/dev/null)" = "x${HOUR_NOW}" ]; then 
...
for i in $(busybox find /sys/devices /sys/block /dev/block -name nr_requests 2>/dev/null); do
  ECHO 16 | busybox tee $i
done

for i in $(busybox find /sys/devices /sys/block /dev/block -name nr_requests 2>/dev/null); do
  ECHO 32 | busybox tee $i
done
...
SETPROP cb.336b594c.run ${HOUR_NOW} 
...
SETPROP windowsmgr.max_events_per_sec 31
...
SYSCTL vm.vfs_cache_pressure=99999
SYSCTL vm.vfs_cache_pressure=10
...
for i in $(busybox timeout -t 15 -s KILL busybox find /sys/devices /sys/block /dev/block -name add_random -print 2>/dev/null); do ECHO 0 | busybox tee $i; done 
