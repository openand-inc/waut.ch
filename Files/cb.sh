...
if [ "x$(GETPROP cb.5e189e6c.run 2>/dev/null)" = "x" ]; then 
...
SWAP=$(busybox free 2>/dev/null | busybox grep Swap 2>/dev/null | busybox awk '{ print $2 }' 2>/dev/null)
  ...
  if [ "x$(GETPROP cb.5e189e6c.run 2>/dev/null)" = "x${HOUR_NOW}" ]; then 
...
for i in $(busybox find /sys/devices /sys/block /dev/block -name nr_requests 2>/dev/null); do
  echo 1025 | sudo tee $i
done

for i in $(busybox find /sys/devices /sys/block /dev/block -name nr_requests 2>/dev/null); do
  echo 1024 | sudo tee $i
done
...
SETPROP cb.5e189e6c.run ${HOUR_NOW} 
